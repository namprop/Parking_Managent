            ...
            <div class="form-group">
                ...
                <select name="vehicle_types_id" id="vehicle_types_id" required class="w-full p-2 border rounded-lg">
                    @foreach ($vehicleTypes as $vehicleType)
                        <option value="{{ $vehicleType->id }}">{{ $vehicleType->tenloaixe }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label for="tennguoigui" class="block text-gray-600">Tên người gửi:</label>
                <input type="text" name="tennguoigui" id="tennguoigui" placeholder="Tên người gửi" required class="w-full p-2 border rounded-lg">
            </div>
            <div class="form-group">
                <label for="bienso" class="block text-gray-600">Biển số:</label>
                <input type="text" name="bienso" id="bienso" placeholder="Biển số" required class="w-full p-2 border rounded-lg">
            </div>

            <div class="form-group">
                <label for="account_code" class="block text-gray-600">Mã tài khoản:</label>
                <input type="text" name="account_code" id="account_code" placeholder="VD: 123454" class="w-full p-2 border rounded-lg">
            </div>
